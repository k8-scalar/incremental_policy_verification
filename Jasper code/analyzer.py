        ...
        if isinstance(obj, Policy):
            if event['custom'] == "create":
                ...
                if is_matrix_all_zero(deltakano):
                    print("\nThe new netowrk policy does not trigger any new container connections")
            ...
            if event['custom'] == "create":
                ...
                for j, i in new_reach.dict_pods.items():
                    if new_reach.matrix[obj.matrix_id][i.matrix_id] == 1:
                        ...
                        print(f"The new pod {obj.name} is deployed on node {nodeName1} and can communicate to pod {i.name} on node {nodeName2} according to the following NetworkPolicies\n")
                    ...
                    if not i.id == obj.id and new_reach.matrix[i.matrix_id][obj.matrix_id] == 1:
                        ...
                        print(f"pod {i.name} on node {nodeName2} can communicate to the new pod {obj.name} which is deployed on node {nodeName1} according to the following NetworkPolicies\n")
                        for (index1, index2) in new_reach.resp_policies.get_items(i.id, obj.id):
                            policy1 = new_reach.dict_pols[index1]
                            print(f"  - NetworkPolicy {policy1.name}")
                            policy2 = new_reach.dict_pols[index2]
                ...
                for j, i in self.kic.reachabilitymatrix.dict_pods.items():
                    if self.kic.reachabilitymatrix.matrix[obj.matrix_id][i.matrix_id] == 1:
                        ...
                        print(f"\nThe deleted pod {obj.name} was deployed on node {nodeName1} and could communicate to pod {i.name} on node {nodeName2} according to the following NetworkPolicies\n")
